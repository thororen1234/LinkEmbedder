...
async function handleTweet(c: Context, tweetId: string, routeUser?: string, embedIndex = -1): Promise<Response> {
  ...
  const customSiteName = `Twitter${dateStr}`;
  ...
  if (photos.length) {
    if (embedIndex >= 0) {
      ...
      return c.html(buildEmbedHtml({ description: desc, url: tweetUrl, imageUrl: photo.url, imageWidth: photo.width, imageHeight: photo.height, color: TWITTER_COLOR, siteName: customSiteName, largeImage: true, oembedUrl }));
    } else if (photos.length > 1) {
      const imageUrls = photos.slice(0, 4).map(p => p.url);
      const first = photos[0];
      return c.html(buildEmbedHtml({ description: desc, url: tweetUrl, imageUrl: imageUrls, imageWidth: first.width, imageHeight: first.height, color: TWITTER_COLOR, siteName: customSiteName, largeImage: true, oembedUrl }));
    } else {
      const first = photos[0];
      return c.html(buildEmbedHtml({ description: desc, url: tweetUrl, imageUrl: first.url, imageWidth: first.width, imageHeight: first.height, color: TWITTER_COLOR, siteName: customSiteName, largeImage: true, oembedUrl }));
    }
  }

  return c.html(buildEmbedHtml({ description: desc, url: tweetUrl, color: TWITTER_COLOR, siteName: customSiteName, oembedUrl }));
}
